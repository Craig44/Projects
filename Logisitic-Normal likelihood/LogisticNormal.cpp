...
double NLLlogistnorm(vector<vector<double>> obs,vector<vector<double>> Exp,vector<double> N,  double sigma, vector<double> phi ,bool sepbysex,bool sexlag, bool robust,bool ARMA,bool sexed) {
  ...
  if (sexed) {
    Kmat.resize(N_ages * 2.0, vector<double>(N_ages * 2.0,0.0));
    Vmat.resize(N_ages * 2.0, vector<double>(N_ages * 2.0,0.0));
  } else {
...
vector<vector<double> > covmat_logistnorm(double sigma, vector<double> phi,unsigned N_ages , bool sepbysex, bool sexlag, bool ARMA, bool sexed) {
  ...
  if (sexed)
    covar.resize((double)N_ages * 2.0, vector<double>((double)N_ages * 2.0,0.0));
  else
    covar.resize(N_ages, vector<double>((double)N_ages,0.0));
  ...
  if (phi[0] == 0.0) {
         ...
         for (int row = 0; row < N_ages; ++ row) {
           for (int col = 0; col < N_ages; ++ col) {
             if (fabs(row - col) == diag + 1)
               covar[row * (i + 1)][col * (i + 1)] = rho[diag] * sigma * sigma;
           }
         }
      }
    }
    ...
    for (int i = 0; i < 1; ++i) {
      ...
      vector<double> col_vec, row_vec, offset_vec;
      for (int row = 0; row < N_ages; ++row) {
        for (int col = 0; col < N_ages; ++col) {
          col_vec.push_back((double)row);
          row_vec.push_back((double)col);
        }
      }
      for (int row = 0; row < N_ages; ++row) {
        for (int col = 0; col < N_ages; ++col) {
      ...
      for (int index = 0; index < N_ages; ++index) {
        for (int index2 = 0; index2 < offset_vec.size(); ++index2) {
          if (index == offset_vec[index2]) {
            vector<int> indexes = get_mat_index(covar.size(), covar[0].size(), offset_vec[index2]);
            covar[indexes[0]][indexes[1]] = rho[index];
          }
        }
      }
